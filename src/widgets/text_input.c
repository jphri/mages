...
tinput_draw(UIObject *input, Rectangle *rect)
{
	...
	gfx_push_texture_rect(gfx_white_texture(), rect->position, rect->half_size, 0, (vec4){ 1.0, 1.0, 1.0, 1.0 });
	gfx_push_font(FONT_ROBOTO, text_position, 12/32.0, (vec4){ 0.0, 0.0, 0.0, 1.0 }, view);
	gfx_push_texture_rect(gfx_white_texture(), caret_position, (vec2){ 1.0, 6 }, 0.0, (vec4){ 0.0, 0.0, 0.0, 1.0 });
	gfx_pop_clip();
}
