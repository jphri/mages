...
button_draw(UIObject *obj, UIEvent *event, Rectangle *rect)
{
	...
	if(ui_get_active() == obj) {
		gfx_push_texture_rect(gfx_white_texture(), rect->position, rect->half_size, (vec2){ 1.0, 1.0 }, 0.0, (vec4){ 0.4, 0.4, 0.4, 1.0 });
	} else if(ui_get_hot() == obj) {
		gfx_push_texture_rect(gfx_white_texture(), rect->position, rect->half_size, (vec2){ 1.0, 1.0 }, 0.0, (vec4){ 0.8, 0.8, 0.8, 1.0 });
	} else 
		gfx_push_texture_rect(gfx_white_texture(), rect->position, rect->half_size, (vec2){ 1.0, 1.0 }, 0.0, (vec4){ 0.6, 0.6, 0.6, 1.0 });
